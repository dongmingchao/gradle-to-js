...
var SPECIAL_KEYS = {
  ...
  maven(chunk, state, old = []) {
    return old.concat(deepParse(chunk, state, true, false));
  },
};
  ...
  for (; state.index < chunkLength; state.index++) {
    ...
    if (character === CHAR_BLOCK_START) {
      ...
      if (SPECIAL_KEYS.hasOwnProperty(currentKey)) {
        out[currentKey] = SPECIAL_KEYS[currentKey](
          chunk,
          state,
          out[currentKey],
        );
      } else if (out[currentKey]) {
        const sub = deepParse(chunk, state, keepFunctionCalls, skipEmptyValues);
        out[currentKey] = deepAssign({}, out[currentKey], sub);
      } else {
...
function findDependencyItemBlock(data) {
  ...
  if (matches && matches[2]) {
    ...
    if (matches[1] === 'compile') {
      matches[2] = 'compile ' + matches[2];
    }
...
function trimWrappingQuotes(str) {
